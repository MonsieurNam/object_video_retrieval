...
IMAGES_PER_ROW = 5
ROWS_PER_SHEET = 8
THUMBNAIL_MAX_W, THUMBNAIL_MAX_H = 480, 270
...
def read_frame_annotated_v7(pool, df_idx, video_name, frame_id, question_id):
    ...
    ok, frame = cap.read()
    if not ok: return None # Không đóng cap ở đây, để pool quản lý
...
def make_thumbnail(img, max_w, max_h):
    h, w = img.shape[:2]; scale = min(max_w / w, max_h / h)
    nw, nh = int(w * scale), int(h * scale)
    resized = cv2.resize(img, (nw, nh), interpolation=cv2.INTER_AREA)
    canvas = np.zeros((max_h, max_w, 3), dtype=np.uint8)
    y0, x0 = (max_h - nh) // 2, (max_w - nw) // 2
    canvas[y0:y0+nh, x0:x0+nw] = resized; return canvas

def draw_tile_label(img, text):
    h, w = img.shape[:2]; font, scale, thick = cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1
    (tw, th), _ = cv2.getTextSize(text, font, scale, thick)
    bar_h = th + 8; y1 = h - bar_h if LABEL_POS == "bottom" else 0; y2 = h if LABEL_POS == "bottom" else bar_h
    overlay = img.copy(); cv2.rectangle(overlay, (0, y1), (w, y2), (0, 0, 0), -1)
    cv2.addWeighted(overlay, LABEL_ALPHA, img, 1 - LABEL_ALPHA, 0, dst=img)
    cv2.putText(img, text, (5, y2 - 5), font, scale, (255, 255, 255), thick, cv2.LINE_AA); return img
...
def write_html_index(q_dir, pages, title):
    ...
    html += "</body></html>"; (q_dir / "index.html").write_text(html, encoding="utf-8")

# --- 3. SCRIPT ĐIỀU PHỐI CHÍNH ---
def generate_visual_report_sequential():
    print("=== TẠO BÁO CÁO TRỰC QUAN V7 (Tuần tự) ===")
    ...
    try:
        for q_id, results in submission.items():
            ...
            all_thumbnails = []
            for (video_name, fid) in frames_to_process:
            ...
            for page_idx in range(n_pages):
                batch_thumbnails = all_thumbnails[page_idx*total_per_sheet : (page_idx+1)*total_per_sheet]
...
if __name__ == "__main__":
    generate_visual_report_sequential()