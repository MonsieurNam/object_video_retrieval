...
# --- 3. V8 QUERY FUNCTIONS ---
# Các hàm này hoạt động trên cột 'consistent_class_name'
...
def query_8(df: pd.DataFrame, conf: float) -> dict:
    ...
    # Đếm số track_id duy nhất cho mỗi class nhất quán trên từng frame
    counts = reliable_df.groupby(['video_name', 'frame_id'])['consistent_class_name'].value_counts().unstack(fill_value=0)
...
if __name__ == "__main__":
    ...
    if not DB_PATH.exists():
        ...
        TEST_CONF = 0.25 # Ngưỡng cơ bản có thể đặt thấp vì dữ liệu rất nhất quán
        
        queries_to_test = {
            "Q1: Người & Xe máy": query_1,
            "Q4: ĐÚNG 1 Người & 1 Xe đạp": query_4,
            "Q6: >1 Người": query_6,
        ...
        for name, func in queries_to_test.items():
            print(f"\n--- Testing: {name} ---")
            # Truyền vào DataFrame đã được xử lý
