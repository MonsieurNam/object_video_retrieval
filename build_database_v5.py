...
OUTPUT_DB_PATH = Path('./evidence_database_v5_expert.feather')
...
# Các lớp quan tâm và text prompts
TARGET_CLASSES = {
...
def build_expert_database_fast():
    start_time = time.time()
    print("--- BẮT ĐẦU XÂY DỰNG CSDL V5 (Tối ưu Tốc độ) ---")
    ...
    for video_path in video_files:
        ...
        with tqdm(total=total_frames, desc="  -> Frames") as pbar:
            ...
            while cap.isOpened():
                ...
                results = yolo.predict(frame, conf=RAW_CONFIDENCE_THRESHOLD, classes=TARGET_CLASS_IDS, 
    ...
    print(f"Lưu CSDL V5 thành công tại: {OUTPUT_DB_PATH}")
    
    end_time = time.time()
    print(f"\n--- GIAI ĐOẠN 1 (V5) HOÀN TẤT. Tổng thời gian: {(end_time - start_time) / 60:.2f} phút. ---")

if __name__ == "__main__":
    build_expert_database_fast()
