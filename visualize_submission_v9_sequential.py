...
OUTPUT_DIR = Path('./visual_report_v9_FINAL/')

# --- Tham số Logic (PHẢI ĐỒNG BỘ VỚI query_library_v9.py) ---
BASE_CONF = 0.25
# Lưu ý: Các ngưỡng này sẽ được áp dụng trên cột 'adjusted_confidence'
...
def draw_detections_v9(frame_bgr, det_rows, question_id, track_colors):
    ...
    for _, det in det_rows.iterrows():
        # Dùng adjusted_confidence để quyết định có vẽ hay không
        if det['adjusted_confidence'] < conf_threshold: continue
            
        x1, y1, x2, y2 = [int(c) for c in det['bbox']]
        inferred_cls, original_cls = det['consistent_class_name'], det['class_name']
        adj_conf, track_id = det['adjusted_confidence'], int(det['track_id'])
        
        color = track_colors[track_id % len(track_colors)].tolist()
        
        label = f"T{track_id}:{inferred_cls}:{adj_conf:.2f}"
...
def read_frame_annotated_v9(pool, df_idx, video_name, frame_id, question_id, track_colors):
    ...
    if rows is not None and not rows.empty:
        frame = draw_detections_v9(frame, rows, question_id, track_colors)
        
    return cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

def make_thumbnail(img, max_w, max_h):
    if img is None: return np.zeros((max_h, max_w, 3), dtype=np.uint8)
    h, w = img.shape[:2]
...
def generate_visual_report_v9_sequential():
    print("=== GENERATING V9 VISUAL REPORT (Sequential & Stable) ===")
    ...
    # *** TIỀN XỬ LÝ V9 NGAY TẠI ĐÂY ***
    print("Preprocessing database: Voting for consistent track identity and adjusting confidence...")
    ...
    track_stats = df.groupby('track_id')['confidence'].agg(['max']).rename(columns={'max': 'track_max_conf'})
        ...
        for q_id, results in submission.items():
            ...
            for page_idx in range(n_pages):
                ...
                for (video_name, fid) in batch:
                    img = read_frame_annotated_v9(pool, df_idx, video_name, fid, q_id, track_colors)
                    thumb = make_thumbnail(img, THUMBNAIL_MAX_W, THUMBNAIL_MAX_H)
                    if SHOW_TILE_LABEL:
...
if __name__ == "__main__":
    generate_visual_report_v9_sequential()